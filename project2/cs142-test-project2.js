...
// Enclose all test cases in a "function enclosure".
// The function closure returns a dictionary which stores p1,p2 and p3Message.
// The returned dictionary is the "window.cs142Project2Results" which is left global.
window = (function() {
    ...
    if (typeof cs142MakeMultiFilter !== 'function') {
        ...
        if (typeof filterFunc !== 'function') {
            ...
            // passing no argument, return originaol array
            ...
            // check that array is filtered, and callback is called
            ...
            // passing no callback, filter as normal
            result =  filterFunc(function (item) {
                return item !== 3;
            });
            ...
            // if passing no filter criterion, do not modify array
            result =  filterFunc();
            ...
            // check that that chained filter work properly
            var arrayFilterer2 = cs142MakeMultiFilter([1,2,3]);
            var currentArray2 = arrayFilterer2(filterTwos)(filterThrees)();
            if (!arraysAreTheSame([1],result)) {
                console.error('chained filter failed' , result);
                p1Message = 'FAILURE';
            }

            // 'this' should refer to the original array
            ...
            // check that filterFunc is not affected by filterFuncTwo
            result =  filterFunc();
            if (!arraysAreTheSame([1],result)) {
                console.error('filterFunc is affected by filterFuncTwo' , result);
                p1Message = 'FAILURE';
            }
            ...
            result =  filterFunc();
            if (!arraysAreTheSame([],result)) {
                console.error('filter function callback does not empty correctly' , result);
                p1Message = 'FAILURE';
            }

            // filter empty array, array is still empty
            result =  filterFunc(function (item) {
                return item !== 0;
            });
            result =  filterFunc();
            if (!arraysAreTheSame([],result)) {
                console.error('empty array crashed' , result);
                p1Message = 'FAILURE';
            }
            ...
            if (!arraysAreTheSame([1, 3], arrayFilterer3(filterTwos)()) || !arraysAreTheSame([4, 5, 6],arrayFilterer4(filterThrees)()) ) {
                console.error('Multiple active filters failed' , result);
                p1Message = 'FAILURE';
            }
        }
    }
    ...
    if (typeof Cs142TemplateProcessor !== 'function') {
        ...
        // var pro = dateTemplate.prototype;
        ...
        if (str !== 'My favorite month is  but not the day 1 or the year 2016') {
            console.error('Cs142TemplateProcessor missing key handling error');
            p2Message = 'FAILURE';
        }
